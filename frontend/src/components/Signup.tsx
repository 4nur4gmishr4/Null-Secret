...
export default function Signup() {
    ...
    return (
        <div className="min-h-[calc(100vh-80px)] w-full flex p-4 font-sans slide-up overflow-y-auto" style={{ background: 'var(--bg-primary)', color: 'var(--text-primary)' }}>
            <div className="hidden lg:flex lg:w-1/2 relative rounded-3xl overflow-hidden p-10 flex-col justify-between" style={{ background: 'var(--bg-secondary)', border: '1px solid var(--border-default)' }}>
                ...
                <div className="relative z-10">
                    <h1 className="text-5xl lg:text-6xl font-bold leading-[1.1] tracking-tight">
                        Zero-Knowledge <br /> Vault Creation
                    </h1>
                </div>
            </div>
            ...
            <div className="w-full lg:w-1/2 flex items-center justify-center p-6 sm:p-12">
                <div className="w-full max-w-[380px] space-y-6">
                    <h2 className="text-[32px] font-bold tracking-tight mb-8">Sign up</h2>

                    {error && <div className="text-sm p-3 border" style={{ color: 'var(--text-danger)', borderColor: 'var(--text-danger)' }}>{error}</div>}

                    <div className="space-y-3">
                        <button onClick={handleGoogleSignUp} type="button" className="w-full flex items-center justify-center gap-2.5 py-3 bg-transparent rounded-none transition-colors text-[15px] font-medium hover:opacity-80" style={{ border: '1px solid var(--border-default)' }}>
                    ...
                    <div className="text-center flex flex-col space-y-4 pt-2 text-[14px]">
                        <p style={{ color: 'var(--text-tertiary)' }}>
                            Already have an account? <Link to="/login" className="font-medium underline underline-offset-4 ml-1" style={{ color: 'var(--text-secondary)' }}>Sign in</Link>
                        </p>
                    </div>
                </div>
            </div>

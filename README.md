...
└────────────────────────────┬────────────────────────────────────────┘
                             │
                             ▼
┌─ Go API (chi router, in-memory only, no database) ──────────────────┐
│                                                                     │
│   ┌─────────────────────────────────────────────────────────────┐   │
│   │ 256 sharded sync.Map (FNV-1a hash) + per-shard mutexes       │   │
│   │ Background sweeper deletes expired entries every minute     │   │
│   │ Token-bucket rate limiter (10 req/min/IP, sliding window)   │   │
...
- The Go server holds ciphertext in RAM. There is no Redis, no SQL,
  no S3, no log file with payloads. A reboot wipes everything.
...
│   │   └── store/                    # Sharded map, GC, rate limiter, tests
│   ├── go.mod
│   ├── go.sum
...
| Rate limiting             | 10 req/min/IP, sliding-window token bucket                        |
| Storage                   | RAM only, sharded `sync.Map`, atomic GC on TTL or view-limit hit  |
